...
def get_n_member_of_geom_prog(number, multiplier, member_count):
    if member_count <= 0:
        return number
    if member_count > 0:
        return get_n_member_of_geom_prog(number * multiplier, multiplier, member_count - 1)
...
if check_for_correct_data(input_base_of_prog) and check_for_correct_data(input_multiplier) and input_count.isdigit():
    ans = get_n_member_of_geom_prog(float(input_base_of_prog), float(input_multiplier), int(input_count))
